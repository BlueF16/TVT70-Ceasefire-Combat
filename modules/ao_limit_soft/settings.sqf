...
{ADDAOMARKER(west,_x)
} forEach ((getMissionLayerEntities "BLUFORAO") select 1);

{ADDAOMARKER(east,_x)
} forEach ((getMissionLayerEntities "OPFORAO") select 1);

ADDAOMARKER(west, "ARTYAOLIMIT");

ADDAOMARKER(east, "MORTARAOLIMIT");
